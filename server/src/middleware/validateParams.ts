...
export function validateDeliverableParams(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  ...
  if (!SAFE_SEGMENT.test(deliverableId) || !SAFE_SEGMENT.test(version)) {
    res.status(400).json({
      error:
        "deliverableId and version must contain only letters, numbers, dots, underscores, and hyphens",
    })
    return
  }
...
export function validateVideoParams(
  ...
  if (
    ...
    !SAFE_SEGMENT.test(deliverableId) ||
    !SAFE_SEGMENT.test(version) ||
    !SAFE_SEGMENT.test(filename)
  ) {
    res.status(400).json({
      error:
        "projectId, deliverableId, version, and filename must contain only letters, numbers, dots, underscores, and hyphens",
    })
    return
  }
