...
import type { SmtpConnectionConfig } from "./smtp-transport.js"
import { SMTP_DEFAULT_TIMEOUT_MS } from "./smtp-transport.js"

export const MAILPIT_DEFAULT_SMTP_PORT = 1025
export const MAILPIT_DEFAULT_FROM_EMAIL = "dev@localhost"
...
export function resolveMailpitSmtpPort(): number {
  ...
  if (!override) {
    return MAILPIT_DEFAULT_SMTP_PORT
  }
...
export function buildMailpitSmtpConnectionConfig(
  apiUrl: string,
  fromEmail = MAILPIT_DEFAULT_FROM_EMAIL,
): SmtpConnectionConfig {
  return {
    host: resolveMailpitSmtpHost(apiUrl),
