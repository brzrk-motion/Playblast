...
describe("GET /video/:projectId/:version/:filename", () => {
  it("returns the full file with Accept-Ranges and Content-Length", async () => {
    const response = await fetch(
      `${baseUrl}/video/${projectId}/${version}/${filename}`,
    )
  ...
  it("returns 404 when the video does not exist", async () => {
    const response = await fetch(
      `${baseUrl}/video/${projectId}/${version}/missing.mp4`,
    )
  ...
  it("returns 400 for invalid path segments", async () => {
    const response = await fetch(
      `${baseUrl}/video/${projectId}/${version}/bad%20name.mp4`,
    )
...
describe("GET /video/:projectId/:version/:filename large file", () => {
  ...
  after(() => {
    fs.rmSync(path.join(getUploadDir(projectId, version), largeFilename), {
      force: true,
    })
  })
  ...
  it("seeks into a 512MB file without reading the entire file", async () => {
    const response = await fetch(
      `${baseUrl}/video/${projectId}/${version}/${largeFilename}`,
